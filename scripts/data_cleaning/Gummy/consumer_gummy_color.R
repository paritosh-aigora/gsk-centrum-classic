...
#temp <- prepped_wide_usage_generic[1896:nrow(prepped_wide_usage_generic),]

prepped_wide_usage_generic <- prepped_wide_usage_generic[1:1895,]
...
prepped_wide_usage_color <- prepped_data_usage %>% 
  ...
  pivot_longer(cols = !ID & !Product,names_to = "Name", values_to = "Value") %>% 
  filter(Value != "No Response") %>%
  mutate(Code = unlist(map(str_split(Value, pattern = "="),function(l) l[1]))) %>% 
  mutate(Value = unlist(map(str_split(Value, pattern = "="),function(l) l[2]))) %>% 
  mutate(Day = unlist(map(str_split(Name, pattern = "-"), function(l) str_trim(l[1])))) %>% 
  mutate(Evaluation_number = unlist(map(str_split(Name, pattern = "-"), function(l) str_trim(l[2])))) %>% 
  mutate(Question = unlist(map(str_split(Name, pattern = "-"), function(l) str_trim(l[3])))) %>% 
  select(ID, Product, Day, Evaluation_number, Question, Value,Code) %>% 
  pivot_wider(names_from = "Question", values_from = c("Value","Code")) %>% 
  drop_na()

color_code <- "Code_What is the color of the gummies you tried today?"
flavor_description <- "Value_How much do you LIKE or DISLIKE the FLAVOR of each gummy?"
#color_description <- Value_What is the color of the gummies you tried today?

gummy_color <- file.path("input", "match_files", "gummy_color_matching.xlsx") %>% 
  read.xlsx(sheet = 2)

prepped_wide_usage_color <- prepped_wide_usage_color %>% 
  select(-color_code, -flavor_description) %>% 
  rename(color = "Value_What is the color of the gummies you tried today?") %>%  
  rename(flavor_opinion = "Code_How much do you LIKE or DISLIKE the FLAVOR of each gummy?") %>% 
  mutate(flavor_opinion = as.numeric(flavor_opinion)) %>% 
  select(-Day, -Evaluation_number) %>% 
  group_by(ID,Product,color) %>% 
  summarize_at(vars(flavor_opinion), mean) %>% 
  ungroup()

prepped_wide_usage_color <- prepped_wide_usage_color %>% 
  right_join(prepped_wide_usage_generic, by = "ID") %>% 
  select(-Product.y) %>% 
  mutate(Product = str_c(Product.x, " (", color, ")" )) %>% 
  select(-color, -Product.x) %>%
  select(ID, Product, everything()) %>% 
  filter(Product %in% gummy_color$Consumer)
...
cons_data <- prepped_wide_usage_color %>%
  left_join(age, by = c("ID" = "ID"), suffix = c("_generic","age")) %>%
  mutate(`SAMPLE AGE` = unlist(map(str_split(`SAMPLE AGE`, pattern = "="),function(l) l[1]))) %>% 
  mutate_all(as.character) %>% 
  mutate(Flavor_Opinion = as.double(flavor_opinion)) %>% 
  mutate(Overall_Opinion = as.double(Overall_Opinion)) %>% 
  select(-flavor_opinion) %>% 
  ungroup()


source(file.path("scripts","data_cleaning","Gummy","relevance_selection_script.R"))
...
data_path <- file.path(
  "input",
  "imported_data",
  "gummy_color_relevance_imported_data.rds"
)
